...
export default function Faculty_Files() {
  ...
  const renderFacultyFile = (title, index) => (
    <div className='w-100 h-15 flex mr-15 mt-5 bg-gray-200 text-[#540C0F] border border-[#540C0F] rounded-xl relative max-lg:mr-10 max-md:w-60 max-md:h-60 max-md:flex-col max-md:items-center max-md:justify-items-center'>
      {/* ملف أيقونة */}
      ...
      <div className='w-[18%] flex justify-center gap-x-3 relative max-md:w-full max-md:h-[20%] max-md:mt-2' ref={el => menuRefs.current[index] = el}>
        ...
        {/* القائمة المنبثقة */}
        {menuIndex === index && (
          <div className="absolute top-full right-0 mt-2 bg-white border border-[#540C0F] rounded-lg shadow-md z-50 w-60 overflow-hidden more-menu max-md:right-auto max-md:left-0">
  ...
  const renderFacultyMember = (name) => (
    <div className='flex mb-2 justify-between items-center w-50 h-10 py-1 px-2 bg-[#8F2125] opacity-70 mt-5 mr-20 rounded-3xl cursor-pointer hover:opacity-100 transition-all duration-200 relative max-lg:mr-15 max-md:w-50 max-md:mx-auto'>
      ...
      <h2 className='text-lg text-white pr-5 max-md:text-sm max-md:pr-8'>{name}</h2>
      <i className='fa-solid fa-chevron-right text-white text-sm mt-1 transition-transform duration-300'></i>
    </div>
  );
        ...
        <div className={`${open ? 'mr-90 max-lg:mr-76' : 'mr-20 max-lg:mr-16'} duration-200 flex-1 justify-items-center`}>
          ...
          <div className="w-full grid grid-cols-1 mb-10">
            ...
            <div className='mr-10 mt-10 mb-5 relative max-lg:mr-5'>
              <div className={`flex gap-x-3 cursor-pointer ${open ? 'faculty_line' : 'faculty_line_close'}`}>
                <i className="fa-solid fa-folder text-[#540C0F] text-4xl relative">
                  <i className='fa-solid fa-chevron-left text-sm text-white absolute top-3 left-3' />
                </i>
            ...
            <div className='mr-10 mt-10 mb-5 relative cursor-pointer max-lg:mr-5'>
              <div className={`flex gap-x-3 ${open ? 'faculty_doctors_line' : 'faculty_doctors_line_close'}`}>
            ...
            {/* عرض أعضاء هيئة التدريس وملفاتهم */}
            {sections.faculty.map((member, memberIndex) => (
              <React.Fragment key={memberIndex}>
                {renderFacultyMember(member.name)}
                {member.files.map((file, fileIndex) => (
                  renderFacultyFile(file.title, `faculty-${memberIndex}-${fileIndex}`)
                ))}
              </React.Fragment>
            ))}
